(function() {
    ...
    /**
     * Mostra un toast notification temporaneo
     */
...
// --- Sezione 3: Esecuzione e Monitoraggio (MutationObserver) ---
     ...
     */
    function runInjections(showNotification = false) {
        const selectCount = injectSelectOptions();
        const inputCount = injectHiddenInputs();
        
        if (showNotification && (selectCount > 0 || inputCount > 0)) {
            showToast(`✓ Injected ${selectCount} operators in select, ${inputCount} hidden inputs`);
        }
    }
    ...
    if (targetSelect && targetInputContainer) {
        ...
        const callback = function(mutationsList, observer) {
            ...
            runInjections(false); // Don't show notification on re-injections
        };
        ...
        runInjections(true); // Show notification on initial injection

    } else {
        // Fallback: se i target non sono disponibili subito, prova dopo 500ms
        setTimeout(() => runInjections(true), 500);
    }
})();
